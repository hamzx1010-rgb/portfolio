        ...
        adminProducts.innerHTML = products.length
            ? products.map(p => `
                <div class="admin-card">
                    <img src="${p.images && p.images[0] ? p.images[0] : 'https://via.placeholder.com/300x170?text=No+Image'}" alt="${p.name}">
                    <div class="info">
                        <div class="meta">${p.category}</div>
                        <h3>${p.name}</h3>
                        <div class="price">${p.price} DZD</div>
                        <button class="btn-primary" onclick="deleteProduct('${p.id}')" style="width:100%; background: #ef4444; box-shadow: none;">Delete</button>
                    </div>
                </div>
              `).join('')
