...
function renderCategories() {
    ...
    categoryFilter.innerHTML = categories.map(cat => {
        ...
        return `<button class="filter-btn ${cat === currentCategory ? 'active' : ''}" onclick="setCategory('${cat}')">${label}</button>`;
    }).join('');
}
    ...
    return `
        <div class="card" onclick="openModal('${p.id}')">
            ...
            <div class="card-body">
                ...
                <button class="card-btn" onclick="event.stopPropagation(); openModal('${p.id}')">Buy Now</button>
            </div>
        </div>
    `;
}
